...
train_loader = tqdm(DataLoader(train_set, batch_size=batch_size, shuffle=True))
val_loader = tqdm(DataLoader(val_set, batch_size=batch_size, shuffle=False))
...
for epoch in range(num_epoch):
    ...
    for i, data in enumerate(train_val_loader):
        ...
        train_acc += np.sum(np.argmax(train_pred.cpu().data.numpy(), axis=1) == data[1].numpy())
        train_loss += batch_loss.item()
    torch.save(model.state_dict(), "./MyModel/myFirstModel.pth")
        # 將結果 print 出來
